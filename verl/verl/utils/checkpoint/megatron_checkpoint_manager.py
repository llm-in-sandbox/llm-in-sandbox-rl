...
class MegatronCheckpointManager(BaseCheckpointManager):
    ...
    def load_checkpoint(self, local_path: str, hdfs_path: str = None, del_local_after_load=False):
        ...
        # Only load dist checkpoint if we're using dist checkpointing (not HF checkpoint via mbridge)
        state_dict = {}
        if self.use_dist_checkpointing:
            ...
            log_with_rank(f"Skipping dist checkpoint loading, using HF checkpoint via bridge", rank=self.rank, logger=logger)
        ...
        # Skip optimizer/scheduler/rng loading when using HF checkpoint (not available in HF format)
        if self.use_hf_checkpoint:
            log_with_rank(f"Skipping optimizer/scheduler/rng loading (not available in HF checkpoint)", rank=self.rank, logger=logger)
        else:
