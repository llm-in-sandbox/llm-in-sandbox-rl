...
def make_serializable_chat_completions(chat_completions):
    """
    Convert chat_completions to JSON-serializable format.
    Handles ToolCall objects and other non-serializable types.
    
    Args:
        chat_completions: List of message dictionaries that may contain ToolCall objects
        
    Returns:
        List of message dictionaries with all objects converted to JSON-serializable format
    """
    serializable_messages = []
    for msg in chat_completions:
        msg_copy = msg.copy()
        
        # Convert ToolCall objects in tool_calls field
        if 'tool_calls' in msg_copy and msg_copy['tool_calls']:
            ...
            for tc in msg_copy['tool_calls']:
                if hasattr(tc, 'to_dict'):
                    ...
                    # Already a dict or serializable
                    serializable_tool_calls.append(tc)
            msg_copy['tool_calls'] = serializable_tool_calls
        
        # Remove non-serializable debug fields if present
        msg_copy.pop('_raw_model_output', None)
        msg_copy.pop('_parsed', None)
...
class AgentPPOTrainer(RayPPOTrainer):
    ...
    def init_workers(self):
        ...
        self.agent_execution_engine = AsyncAgentExecutionEngine(
            ...
            overlong_step_penalty=self.config.rllm.agent.get("overlong_step_penalty", False),
            disable_thinking=self.config.rllm.disable_thinking,
            tool_parser_type=self.config.rllm.get("tool_parser_type", None),
    ...
    def init_envs_and_agents(self, batch):
        ...
        # Auto-propagate scaffold from agent_args to env_args if not already set
        if "scaffold" in full_agent_args and "scaffold" not in base_env_args:
            base_env_args["scaffold"] = full_agent_args["scaffold"]
    ...
    def _validate_agent(self):
        rewards_lst = []
        original_rewards_lst = []  # Track original rewards (before overlong/step penalty)
        data_source_lst = []
        uid_lst = []
        for test_data in self.val_dataloader:
            ...
            if self.config.rllm.stepwise_advantage.enable:
                ...
                test_output_gen_batch, _ = self.generate_agent_trajectory(meta_info=test_batch.meta_info, is_validation=True)

            test_batch = test_batch.union(test_output_gen_batch)

            reward_tensor = test_batch.batch["token_level_scores"]
            # Also get original_scores if available (stored per trajectory)
            if "original_scores" in test_batch.batch:
                original_rewards_lst.append(test_batch.batch["original_scores"].sum(-1).cpu())
            else:
                # Fallback: use reward_tensor if original_scores not available
                original_rewards_lst.append(reward_tensor.sum(-1).cpu())
            
            rewards_lst.append(reward_tensor.sum(-1).cpu())
            data_source_lst.append(test_batch.non_tensor_batch.get("data_source", ["unknown"] * reward_tensor.shape[0]))
            uid_lst.append(test_batch.non_tensor_batch["uid"])

        reward_tensor = torch.cat(rewards_lst, dim=0)  # (batch_size,) - reward_for_update
        original_reward_tensor = torch.cat(original_rewards_lst, dim=0)  # (batch_size,) - original_reward
        data_sources = np.concatenate(data_source_lst, axis=0)
        # evaluate test_score based on data source
        data_source_reward = {}
        data_source_original_reward = {}  # Track original rewards by data source

        # to group for pass@k
        uid_tensor = np.concatenate(uid_lst, axis=0)
        data_source_uid_pass_rates = {}  # data source to {uid: pass or not}
        data_source_uid_original_pass_rates = {}  # data source to {uid: original pass or not}
        ...
        for i in range(reward_tensor.shape[0]):
            ...
            if data_source not in data_source_reward:
                ...
                data_source_original_reward[data_source] = []
            data_source_reward[data_source].append(reward_tensor[i].item())
            data_source_original_reward[data_source].append(original_reward_tensor[i].item())

            # pass@k
            if data_source not in data_source_uid_pass_rates:
                data_source_uid_pass_rates[data_source] = {}
                data_source_uid_original_pass_rates[data_source] = {}

            uid = uid_tensor[i]
            if uid not in data_source_uid_pass_rates[data_source]:
                data_source_uid_pass_rates[data_source][uid] = 0  # default to not pass
                data_source_uid_original_pass_rates[data_source][uid] = 0
            # take highest score
            data_source_uid_pass_rates[data_source][uid] = max(data_source_uid_pass_rates[data_source][uid], reward_tensor[i].item())
            data_source_uid_original_pass_rates[data_source][uid] = max(data_source_uid_original_pass_rates[data_source][uid], original_reward_tensor[i].item())

        metric_dict = {}
        for data_source, rewards in data_source_reward.items():
            # Log original rewards (before penalty) - this is the actual task score
            original_rewards_array = np.array(data_source_original_reward[data_source])
            original_rewards_array = np.clip(original_rewards_array, 0, 1)
            metric_dict[f"val/score/{data_source}"] = np.mean(original_rewards_array)

        return metric_dict

    def generate_agent_trajectory(self, timing_raw=None, meta_info=None, is_validation=False):
        """
        ...
        Args:
            ...
            is_validation (bool): Whether this is validation data (affects save path).
        ...
        with marked_timer("transform_trajectory", timing_raw):
            ...
            final_gen_batch_output, metrics = self._transform_agent_trajectories(trajectories, is_validation=is_validation)
    ...
    def _transform_agent_trajectories(self, trajectories: list[dict], is_validation: bool = False):
        """
        ...
        all_masks_list = []
        traj_scores = []  # reward_for_update (after penalty)
        original_scores = []  # original_reward (before penalty)
        chat_completions = []
        entries = []  # Collect entry information
        traj_metrics = []
        metrics = {}
        ...
        for traj in trajectories:
            ...
            # Get original_reward from metrics, fallback to trajectory_reward if not available
            traj_metric = traj["metrics"]
            original_scores.append(traj_metric.get("original_reward", traj["trajectory_reward"]))
            chat_completions.append(traj["chat_completions"])
            entries.append(traj.get("entry", None))  # Get entry if available
            traj_metrics.append(traj_metric)
        ...
        # Save chat completions to a file with rewards
        if is_validation:
            save_dir = os.path.join(self.config.trainer.default_local_dir, "chat_completions")
            os.makedirs(save_dir, exist_ok=True)
            save_path = os.path.join(save_dir, f"val_{self.global_steps}.jsonl")
        else:
            save_dir = os.path.join(self.config.trainer.default_local_dir, "chat_completions")
            os.makedirs(save_dir, exist_ok=True)
            save_path = os.path.join(save_dir, f"{self.global_steps}.jsonl")
        
        # Use append mode for validation to avoid overwriting across batches
        file_mode = "a" if is_validation else "w"
        with open(save_path, file_mode) as f:
            for i, chat_completion in enumerate(chat_completions):
                # Add reward information to each chat_completion before saving
                # Convert numpy types to Python native types for JSON serialization
                reward_value = traj_scores[i]
                ...
                output_data = {
                    "chat_completions": make_serializable_chat_completions(chat_completion),
                    "trajectory_reward": reward_value,
                }
                
                # Add entry if available
                if entries[i] is not None:
                    output_data["entry"] = entries[i]
        ...
        original_score_batch = torch.zeros_like(response_batch, dtype=torch.float32)
        ...
        for i, score in enumerate(traj_scores):
            ...
            if resp_len > 0 and resp_len <= score_batch.shape[1]:
                ...
                original_score_batch[i, resp_len - 1] = original_scores[i]
        ...
        tensor_batch = {
            ...
            "original_scores": original_score_batch,  # original reward before penalty
    ...
    def visualize_trajectory(self, tensor_batch, sample_idx=0, max_samples=1, mask_key="response_mask"):
        ...
        # Calculate final rewards and categorize samples
        final_rewards = token_level_scores.sum(dim=-1)  # Sum rewards across tokens
        reward_one_indices = [i for i in range(batch_size) if abs(final_rewards[i].item() - 1.0) < 1e-6]
        reward_zero_indices = [i for i in range(batch_size) if abs(final_rewards[i].item()) < 1e-6]
        reward_partial_indices = [i for i in range(batch_size) if 1e-6 < final_rewards[i].item() < 1.0 - 1e-6]
        
        # Select one sample from each category
        selected_indices = []
        categories = []
        
        if reward_one_indices:
            selected_indices.append(reward_one_indices[0])
            categories.append("reward=1")
        if reward_zero_indices:
            selected_indices.append(reward_zero_indices[0])
            categories.append("reward=0")
        if reward_partial_indices:
            selected_indices.append(reward_partial_indices[0])
            categories.append("partial_reward")
        
        if not selected_indices:
            colorful_print("No trajectories found to visualize", fg="yellow")
            return
        
        colorful_print(f"\nVisualizing {len(selected_indices)} samples: {', '.join(categories)}", fg="magenta", bold=True)

        for idx, i in enumerate(selected_indices):
            colorful_print("\n" + "=" * 60, fg="cyan", bold=True)
            colorful_print(f"Sample {idx} [{categories[idx]}] (batch index {i}, final_reward={final_rewards[i].item():.4f})", fg="cyan", bold=True)
