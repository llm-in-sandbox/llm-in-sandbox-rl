...
class VerlEngine(RolloutEngine):
    def __init__(self, config, rollout_manager, tokenizer, **kwargs):
        ...
        self.chat_parser = ChatTemplateParser.get_parser(
            ...
            tool_parser_type=config.get("rllm", {}).get("tool_parser_type", None)
        )
    ...
    async def get_model_response(self, messages: list[dict], **kwargs) -> ModelOutput:
        ...
        # Check if prompt exceeds the model's maximum context length
        # This prevents sending impossibly long prompts to vLLM which would cause negative max_tokens
        max_model_context = self.max_prompt_length + self.max_response_length
        if prompt_length > max_model_context:
            print(f"[WARNING] Prompt length ({prompt_length} tokens) exceeds model context ({max_model_context} tokens). "
                  f"Messages: {len(messages)}. Consider message truncation in agent.")
