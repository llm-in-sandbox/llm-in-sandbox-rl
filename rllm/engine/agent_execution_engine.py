...
class AgentExecutionEngine:
    def __init__(
        ...
        overlong_step_penalty=False,  # Apply zero reward for trajectories that exceeds the max steps/length
        **kwargs,
    ):
        ...
        self.overlong_step_penalty = overlong_step_penalty

        assert (self.overlong_filter and self.overlong_step_penalty) is False, "overlong_filter and overlong_step_penalty cannot be both enabled, overlong_filter will mask out the trajectory making overlong_step_penalty ineffective."
        ...
        self.accumulate_reasoning = self.config.get("rllm", {}).get("accumulate_reasoning", False) if self.config is not None else False
        ...
        if chat_parser is None:
            tool_parser_type = kwargs.get("tool_parser_type", None)
    ...
    async def get_model_response(self, prompt, application_id, **kwargs):
        ...
        Returns:
            ModelOutput object containing text, content, reasoning, tool_calls, etc.
    ...
    async def run_agent_trajectory_async(self, idx, application_id, seed=0, mode="Text", **kwargs):
        ...
        # Get tools for training data conversion
        tools = agent.get_tools() if hasattr(agent, 'get_tools') else None
  
        prompt_tokens, _ = convert_messages_to_tokens_and_masks(messages, tokenizer=self.tokenizer, parser=self.chat_parser, contains_first_msg=True, contains_generation_msg=True, tools=tools, accumulate_reasoning=self.accumulate_reasoning)
        ...
        for step_idx in range(self.max_steps):
            ...
            # Get tools from agent if it has the get_tools method
            # Always add tools to kwargs (can be None) so debugging is consistent
            # tools = agent.get_tools() if hasattr(agent, 'get_tools') else None
            kwargs["tools"] = tools
            ...
            if assistant_message:
                # For training, we use the configured accumulate_reasoning setting (True means include reasoning in training data)
                assistant_msg_tokens, assistant_msg_masks = convert_messages_to_tokens_and_masks([assistant_message], tokenizer=self.tokenizer, parser=self.chat_parser, contains_first_msg=False, contains_generation_msg=False, tools=tools, accumulate_reasoning=True)
            if env_messages:
                # Environment messages don't have reasoning, so accumulate_reasoning=False (default)
                env_msg_tokens, env_msg_masks = convert_messages_to_tokens_and_masks(env_messages, tokenizer=self.tokenizer, parser=self.chat_parser, contains_first_msg=False, contains_generation_msg=True, tools=tools)
            ...
            # Update the token version of trajectory (for training data)
        ...
        # Track both original reward and reward used for update
        original_reward = 0.0
        ...
        if hasattr(env, "compute_final_reward") and not masked_out:
            cur_step = agent.get_current_state()
            start_time = time.time()
            original_reward = await loop.run_in_executor(self.executor, env.compute_final_reward)
            reward_time = time.time() - start_time
            
            if self.overlong_step_penalty and termination_reason and termination_reason in ["MAX_STEPS", "TRUNCATION"]:
                penalized_reward = 0.0
            else:
                penalized_reward = original_reward
            cur_step.reward = penalized_reward
            
        # Closing environment using the executor.
        await loop.run_in_executor(self.executor, env.close)
        
        # Check if this is validation or training
        is_validate = kwargs.get("meta_info", {}).get("validate", False)
        mode_tag = "[VAL]" if is_validate else "[TRAIN]"
        
        if termination_reason:
            color = "green" if original_reward > 0 else "yellow"
            is_overlong = termination_reason in ["MAX_STEPS", "TRUNCATION"]
            if is_validate:
                if is_overlong:
                    log_msg = f"[VAL] Trajectory {idx} completed due to {termination_reason} (overlong) | score={original_reward:.4f}"
                    colorful_print(log_msg, "red")
                else:
                    log_msg = f"[VAL] Trajectory {idx} completed due to {termination_reason} | score={original_reward:.4f}"
                    colorful_print(log_msg, color)
            else:
                if original_reward != penalized_reward:
                    log_msg = f"[TRAIN] Trajectory {idx} completed due to {termination_reason} | reward={original_reward:.4f} -> {penalized_reward:.4f} (overlong penalty)"
                    colorful_print(log_msg, "red")
                else:
                    log_msg = f"[TRAIN] Trajectory {idx} completed due to {termination_reason} | reward={original_reward:.4f}"
                    colorful_print(log_msg, color)
                if masked_out:
                    colorful_print(f"[TRAIN] Trajectory {idx} is masked out due to overlong filter.", "red")
        ...
        if mode == "Text":
            ...
            token_result = {
                ...
                "entry": getattr(env, 'entry', None),  # Add entry if available
                "metrics": {
                    ...
                    # Original reward from environment (before penalty)
                    "original_reward": original_reward,
                    # Reward used for model update (after penalty if applicable)
                    "penalized_reward": penalized_reward,
                    # reward for whole trajectory
                    "trajectory_reward": trajectory.reward,
                },
            }
