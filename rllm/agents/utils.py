...
def convert_messages_to_tokens_and_masks(messages: list[dict[str, str]], tokenizer: PreTrainedTokenizerBase, parser: ChatTemplateParser, contains_first_msg=False, contains_generation_msg=False, tools=None, accumulate_reasoning=False):
    """
    ...
    Args:
        ...
        tools (list): Optional tools to include in the prompt.
        accumulate_reasoning (bool): Whether to include reasoning in assistant messages (for training).
    ...
    def _convert_message_to_tokens_and_masks(msg, first_msg=False, generation_msg=False):
        # Only pass tools to the FIRST message to avoid duplicate tool definitions
        # apply_chat_template automatically generates a system message with tool definitions
        # when tools are provided, so we should only do this once at the beginning
        should_include_tools = tools is not None and first_msg
        msg_text = parser.parse([msg], add_generation_prompt=generation_msg, is_first_msg=first_msg, tools=tools if should_include_tools else None, accumulate_reasoning=accumulate_reasoning)
