...
class Step:
    ...
    def to_dict(self) -> dict:
        # Handle action serialization - if it's an Action object with to_xml_string(), use that
        action_value = self.action
        if hasattr(self.action, 'to_xml_string'):
            action_value = self.action.to_xml_string()
        elif hasattr(self.action, 'to_dict'):
            action_value = self.action.to_dict()
        
        return {
            "chat_completions": self.chat_completions,
            "observation": self.observation,
            "thought": self.thought,
            "action": action_value,  # Use serialized version
